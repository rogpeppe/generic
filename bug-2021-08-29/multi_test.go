...
func TestMultiWriterSingleChainFlatten(t *testing.T) {
	...
	var w Writer[byte] = MultiWriter[byte](writerFunc(func(p []byte) (int, error) {
	...
	for i := 0; i < 100; i++ {
		mw = MultiWriter[byte](w)
	}

	mw = MultiWriter[byte](w, mw, w, mw)
...
func TestMultiReaderFlatten(t *testing.T) {
	...
	var r Reader[byte] = MultiReader[byte](readerFunc(func(p []byte) (int, error) {
		n := runtime.Callers(1, pc)
...
func TestMultiReaderSingleByteWithEOF(t *testing.T) {
	got, err := ioutil.ReadAll(LimitReader[byte](MultiReader[byte](byteAndEOFReader('a'), byteAndEOFReader('b')), 10))
	if err != nil {
		t.Fatal(err)
	}
	const want = "ab"
