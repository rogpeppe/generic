...
func TestCopyNEOF(t *testing.T) {
	...
	n, err := CopyN[byte](&noReadFrom{b}, strings.NewReader("foo"), 3)
	if n != 3 || err != nil {
		t.Errorf("CopyN(noReadFrom, foo, 3) = %d, %v; want 3, nil", n, err)
	}

	n, err = CopyN[byte](&noReadFrom{b}, strings.NewReader("foo"), 4)
	if n != 3 || err != EOF {
	...
	n, err = CopyN[byte](b, wantedAndErrReader{}, 5)
	if n != 5 || err != nil {
		t.Errorf("CopyN(bytes.Buffer, wantedAndErrReader, 5) = %d, %v; want 5, nil", n, err)
	}

	n, err = CopyN[byte](&noReadFrom{b}, wantedAndErrReader{}, 5)
	if n != 5 || err != nil {
...
func TestTeeReader(t *testing.T) {
	...
	if n, err := ReadFull[byte](r, dst); err != nil || n != len(src) {
